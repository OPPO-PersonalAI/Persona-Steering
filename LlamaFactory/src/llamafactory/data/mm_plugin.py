...
# Import make_batched_videos with fallback logic
try:
    # Try importing from video_utils first (newer versions)
    from transformers.video_utils import make_batched_videos
except ImportError:
    # Fallback to image_utils for older versions
    try:
        from transformers.image_utils import make_batched_videos
    except ImportError:
        # If both fail, raise a clear error message
        raise ImportError(
            "Cannot import make_batched_videos from transformers.video_utils or transformers.image_utils. "
            "Please upgrade transformers to a version >= 4.52.0 or check your installation."
        )

# Import make_flat_list_of_images
