...
# Portions of this file are modifications by OPPO.
import json
...
class BaseModelArguments:
    ...
    def __post_init__(self):
        ...
        if self.adapter_name_or_path is not None:  # support merging multiple lora weights
            # Normalize adapter list
            if isinstance(self.adapter_name_or_path, str):
                self.adapter_name_or_path = [path.strip() for path in self.adapter_name_or_path.split(",")]
            elif isinstance(self.adapter_name_or_path, list):
                # Already a list
                self.adapter_name_or_path = [path.strip() if isinstance(path, str) else path for path in self.adapter_name_or_path]

        if self.add_tokens is not None:  # support multiple tokens
            # Normalize add_tokens
            if isinstance(self.add_tokens, str):
                self.add_tokens = [token.strip() for token in self.add_tokens.split(",")]
            elif isinstance(self.add_tokens, list):
                self.add_tokens = [token.strip() if isinstance(token, str) else token for token in self.add_tokens]
...
@dataclass
class PersonaSteeringArguments:
    r"""Arguments for PersonaSteering model configuration."""

    user_encoder_name: str = field(
        default="bert-base-uncased",
        metadata={"help": "User encoder model name (e.g., bert-base-uncased, Qwen/Qwen2-1.5B)."},
    )
    steering_layer_idx: int = field(
        default=-1,
        metadata={"help": "Layer index to inject steering vector (-1 = last layer, 0 = first layer)."},
    )
    steering_coeff: float = field(
        default=1.0,
        metadata={"help": "Coefficient for steering vector strength."},
    )
    user_encoder_use_lora: bool = field(
        default=False,
        metadata={"help": "Whether to use LoRA for user encoder."},
    )
    user_encoder_lora_r: int = field(
        default=8,
        metadata={"help": "LoRA rank for user encoder."},
    )
    user_encoder_lora_alpha: int = field(
        default=16,
        metadata={"help": "LoRA alpha for user encoder."},
    )
    user_encoder_lora_dropout: float = field(
        default=0.05,
        metadata={"help": "LoRA dropout for user encoder."},
    )
    user_encoder_lora_target_modules: Optional[str] = field(
        default=None,
        metadata={
            "help": "Name(s) of target modules to apply LoRA to the user encoder. Use commas to separate multiple modules."
        },
    )
    steering_mlp_activation: str = field(
        default="gelu",
        metadata={"help": "Activation function for steering MLP (gelu, relu, tanh)."},
    )
    steering_mlp_dropout: float = field(
        default=0.1,
        metadata={"help": "Dropout rate for steering MLP."},
    )
    steering_mlp_use_layer_norm: bool = field(
        default=True,
        metadata={"help": "Whether to use LayerNorm in steering MLP."},
    )
    decoder_hidden_size: int | None = field(
        default=None,
        metadata={"help": "Decoder hidden size. If None, will be auto-detected from model config."},
    )
    freeze_decoder: bool = field(
        default=True,
        metadata={"help": "Whether or not to freeze the decoder parameters."},
    )
    use_two_stage_training: bool = field(
        default=False,
        metadata={"help": "Enable two-stage training (Stage 1: MLP only, Stage 2: All)."},
    )
    mlp_only_epochs: int = field(
        default=1,
        metadata={"help": "Number of epochs for Stage 1 (MLP only training)."},
    )
    mlp_only_steps: int | None = field(
        default=None,
        metadata={"help": "Number of steps for Stage 1 (MLP only training). If None, uses mlp_only_epochs * steps_per_epoch."},
    )
    use_mse_trainer: bool = field(
        default=False,
        metadata={"help": "Whether to use MSE trainer for demographic reconstruction training."},
    )
    max_generation_length: int = field(
        default=128,
        metadata={"help": "Maximum length of generated text for MSE trainer."},
    )
    generation_temperature: float = field(
        default=1.0,
        metadata={"help": "Temperature for text generation in MSE trainer."},
    )
    fusion_alpha_min: float = field(
        default=0.1,
        metadata={"help": "Minimum value for fusion gate alpha (demographic weight). Prevents collapse to profile-only."},
    )
    fusion_alpha_max: float = field(
        default=0.9,
        metadata={"help": "Maximum value for fusion gate alpha. Prevents collapse to demographic-only."},
    )
...
class ModelArguments(
    ...
    def __post_init__(self):
        ...
        self.user_encoder_lora_target_modules = split_arg(self.user_encoder_lora_target_modules)
