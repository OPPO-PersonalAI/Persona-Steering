...
class DataArguments:
    ...
    max_user_length: int = field(
        default=512,
        metadata={"help": "Maximum length for user input (with history). Used for PersonaSteering."},
    )
    max_history_ugc: int = field(
        default=3,
        metadata={"help": "Maximum number of UGC items to sample for steering vector. Used for PersonaSteering."},
    )
    max_history_pairs: int = field(
        default=2,
        metadata={"help": "Maximum number of pair-wise feedback items to sample. Used for PersonaSteering."},
    )
    decoder_prompt: str | None = field(
        default=None,
        metadata={"help": "Optional prompt to prepend to decoder input. Used for PersonaSteering."},
    )
    task: str | None = field(
        default=None,
        metadata={"help": "Task type for PersonaSteering dataset processing (e.g., generation_abstract, lamp_4, lamp_5, lamp_7)."},
    )
    max_samples_per_user: int | None = field(
        default=None,
        metadata={"help": "Maximum number of samples (profile items) to use per user in PersonaSteering training. If None, use all profile items."},
    )
    profile_topk_prescreen: bool = field(
        default=False,
        metadata={
            "help": (
                "Whether to pre-screen profile items by query relevance before building "
                "the profile stream in persona_profile_steering."
            )
        },
    )
    profile_topk: int | None = field(
        default=None,
        metadata={
            "help": (
                "Top-k profile items to keep when profile_topk_prescreen is enabled. "
                "If None, fallback to max_profile_items in dataset logic."
            )
        },
    )
